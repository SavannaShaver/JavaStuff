...
public class LinkedBST <T extends Comparable<T>> { // we have this extends etc to make the object comparable
	...
	public void printTheMax()
	{
		printTheMax(root); // prints from the root
	}
	private void printTheMax(Node aNode)
	{
	//going to be recursive
	if(aNode == null) // halting conditions
		return;
	Node temp = findMaxTree(aNode.rightChild); // this finds the l value in this trees right side
		aNode.data = temp.data;
	System.out.println(aNode.data);
	//System.out.println(this.area); extend Shape and you can get it to print 0.0
	}
	private Node findMaxTree(Node aNode) // finds the largest3 value in the tree
	{
	if(aNode == null)
		return null;
		...
		return findMaxTree(aNode.rightChild);
	}
	
	public void pug(T aData)
	{
		root = pug(root,aData);
	}
	
	//need to fix
	private Node pug(Node aNode, T aData) //two children example
	{
		if(aNode == null)
			return null;
		else if(aData.compareTo(aNode.data)<0)// go left
		{
			//aNode.leftChild = remove(aNode.leftChild, aNode.data);
			//remove(aNode.leftChild, aNode.data);
			//remove(aNode.rightChild, aNode.data);
			remove(aNode.data);
			pug(aNode.leftChild, aData);
